...
// this is the end of the jpg decoder library
////////////////////////////////////////////////////////////////////////////////////
...
function render() {
  ...
  var gif = DriveApp.getFolderById(folderID).getFiles();
  var frame = gif.next();
  var frameData = new Uint8Array(frame.getSize());
  frameData = frame.getBlob().getBytes().map(_ => _ < 0 ? 256 + _ : _);
  var image = decode(frameData, {useTArray: true});
  ...
  if(sheet.getMaxColumns() < width){
    sheet.insertColumns(1, width - sheet.getMaxColumns());
  }
  if(sheet.getMaxRows() < height){
    sheet.insertRows(1, height - sheet.getMaxRows());
  }
  const range = sheet.getRange(`A1:${colNumberToLetters(width)}${height}`);
  sheet.setRowHeights(1, height, 11);
  sheet.setColumnWidths(1, width, 10);
  while(true){
    gif = DriveApp.getFolderById(folderID).getFiles();
    while(gif.hasNext()){
      let then = Date.now();
      frame = gif.next();
      frameData = new Uint8Array(frame.getSize());
      frameData = frame.getBlob().getBytes().map(_ => _ < 0 ? 256 + _ : _);
      image = decode(frameData, {useTArray: true});
      let k = 0;
      for(let i = 0; i < image.height; i++){
        for(let j = 0; j < image.width; j++){
          data[i][j] = [image.data[k++], image.data[k++], image.data[k++]];
          k++;
        }
      }
...
function reorder() {
  ...
  for(let i = len-1; i >= 0; --i){
    ...
    while(files.hasNext()){
      file = files.next();
      if(file.getName() === `${curName}${i}.jpg`){
        file.setName(`${newName}${i}.jpg`);
      }
    }
  }
}
...
function clear() {
  let sheet = SpreadsheetApp.openById(sheetsID).getSheets()[0];
  sheet.getRange(`A1:${colNumberToLetters(sheet.getMaxColumns())}${sheet.getMaxRows()}`).setBackground("white")
}
